#!/usr/bin/env python3
"""
Astro-Blogs Navigation Links Generator Utility

This script automatically adds Previous/Next article navigation links to MD/MDX files
based on the order specified in a _meta.json file.

Usage:
    python generate_nav_links.py [folder_path]
...
Output:
    - Appends navigation links to each MD/MDX file in the format:
        ---
        ## Previous Article
        - [Title](/path/to/previous.md)
        ---
        ## Next Article  
        - [Title](/path/to/next.md)
        ---
...
Customization:
    1. Modify the nav_snippet template to change link formatting
    2. Adjust base path in link construction (/blogs-md/)
    3. Change sorting behavior by modifying the 'order' key handling

Example:
    python generate_nav_links.py ./content/planets  # Process planet articles
"""
